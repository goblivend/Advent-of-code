...
part2 (Input instr mps) starting = product te
    where
        findZ :: [((String, String) -> String)] -> Int -> String -> Int
        findZ (e:l) i s
            | "Z" `isSuffixOf` s = i
            | otherwise         =  next
                where
                    next = findZ l (i+1) ((e . (mps !)) s)
        res =  map (findZ (cycle instr) 0) (filter starting $ keys mps)
        primes = sieve [2..]
            where
                sieve (p:xs) = p : sieve [x|x <- xs, x `mod` p > 0]
        multiplicators :: [Int] -> Int -> [Int]
        multiplicators (e:l) n
            | e > n =  []
            | n `mod` e == 0 = e : multiplicators (e:l) (div n e)
            | otherwise =  (multiplicators l n)

        te = uniq . sort . concat $ map (multiplicators primes) res
